...
export class AlertaService implements OnModuleInit {
  ...
  findAll() {
    return (this.prisma as any).$queryRawUnsafe(`SELECT id, titulo, mensaje, severidad, estado, origen, creado_en AS "creadoEn", resuelto_en AS "resueltoEn" FROM seguridad.alerta ORDER BY creado_en DESC`);
  }
  ...
  async create(data: Record<string, unknown>) {
    const rows = await (this.prisma as any).$queryRawUnsafe(
      ...
      data.titulo, data.mensaje, data.severidad ?? 'INFO', data.estado ?? 'ACTIVA', data.origen ?? 'SISTEMA',
    );
    return rows[0];
  }
  ...
  async update(id: string, data: Record<string, unknown>) {
    const rows = await (this.prisma as any).$queryRawUnsafe(
      ...
      id, data.titulo ?? null, data.mensaje ?? null, data.severidad ?? null, data.estado ?? null, data.origen ?? null,
    );
    return rows[0];
  }

  async remove(id: string) {
    await (this.prisma as any).$executeRawUnsafe(`DELETE FROM seguridad.alerta WHERE id=$1::uuid`, id);
    return { deleted: true };
  }
}
